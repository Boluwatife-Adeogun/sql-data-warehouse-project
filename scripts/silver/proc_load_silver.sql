...
SELECT
    ...
    prd_nm,
    COALESCE(NULLIF(prd_cost, '')::INT, 0) AS prd_cost,
    CASE 
    ...
    END AS prd_line,
    prd_start_dt::DATE,
    LEAD(prd_start_dt) OVER (PARTITION BY SUBSTRING(prd_key FROM 7) ORDER BY prd_start_dt) - INTERVAL '1 day' AS prd_end_dt
